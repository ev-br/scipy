...
void __nnls(const CBLAS_INT m, const CBLAS_INT n, double* restrict a, double* restrict b, double* restrict x, double* restrict w, double* restrict zz, CBLAS_INT* restrict indices, const CBLAS_INT maxiter, double* rnorm, CBLAS_INT* info);
static void ldp(CBLAS_INT m, CBLAS_INT n, double* g, double* h, double* x, double* buffer, CBLAS_INT* indices, double* xnorm, CBLAS_INT* mode);
static void lsi(CBLAS_INT me, CBLAS_INT mg, CBLAS_INT n, double* e, double* f, double* g, double* h, double* x, double* buffer, CBLAS_INT* jw, double* xnorm, CBLAS_INT* mode);
static void lsei(CBLAS_INT ma, CBLAS_INT me, CBLAS_INT mg, CBLAS_INT n, double* a, double* b, double* e, double* f, double* g, double* h, double* x, double* buffer, CBLAS_INT* jw, double* xnorm, CBLAS_INT* mode);
static void lsq(CBLAS_INT m, CBLAS_INT meq, CBLAS_INT n, CBLAS_INT augment, double aug_weight, double* Lf, double* gradx, double* C, double* d, double* xl, double* xu, double* x, double* y, double* buffer, CBLAS_INT* jw, CBLAS_INT* mode);
static void ldl_update(CBLAS_INT n, double* a, double* z, double sigma, double* w);
    ...
    CBLAS_INT* indices)
{
    ...
    CBLAS_INT n = S->n;  // XXX: S**_vars members
    CBLAS_INT m = S->m;
    CBLAS_INT n1 = n + 1;
    CBLAS_INT n2 = n1*n/2;
...
MODE0:
    ...
    S->iter = 0;
    S->reset = 0;
    for (CBLAS_INT i = 0; i < n; i++) { s[i] = 0.0; }
    for (CBLAS_INT i = 0; i < m; i++) { mu[i] = 0.0; }

RESET_BFGS:
    // Reset the BFGS matrix stored in packed format
    S->reset++;
    if (S->reset > 5) { goto LABEL255;}
    for (CBLAS_INT i = 0; i < n2; i++) { bfgs[i] = 0.0; }
    j = 0;
...
ITER_START:
    ...
    for (CBLAS_INT i = 0; i < n; i++) { v[i] = gradx[i]; }
    BLAS_FUNC(dgemv)("T", &m, &n, &dmone, C, &lda, mult, &one, &done, v, &one);

    S->f0 = *funx;
    for (CBLAS_INT i = 0; i < n; i++) { x0[i] = sol[i]; }
    S->gs = BLAS_FUNC(ddot)(&n, gradx, &one, s, &one);
    S->h1 = fabs(S->gs);
    S->h2 = 0.0;
    for (CBLAS_INT j = 0; j < m; j++)
    {
        if (j < S->meq)
        {
            S->h3 = d[j];
...
LINE_SEARCH:
    ...
    for (CBLAS_INT i = 0; i < n; i++) { sol[i] = x0[i]; }
...
MODEM1:
    ...
    for (CBLAS_INT i = 0; i < n; i++) { u[i] = gradx[i]; }
    ...
    // L'*S
    for (CBLAS_INT i = 0; i < n; i++) { v[i] = s[i]; }
...
void lsq(
    ...
    double* restrict y, double* buffer, CBLAS_INT* jw, CBLAS_INT* mode)
{
    ...
    for (CBLAS_INT i = 0; i < (ld+2)*ld; i++) { buffer[i] = 0.0; }
    ...
    for (CBLAS_INT i = 0; i < n; i++) { wb[i] = gradx[i]; }
    ...
    if (meq > 0)
    {
        for (CBLAS_INT j = 0; j < n-1; j++)
        {
            for (CBLAS_INT i = 0; i < meq; i++)
            {
                wE[i + j*meq] = C[i + j*m];
            }
        }
        if (augment)
        {
            ...
            for (CBLAS_INT i = 0; i < meq; i++) { wE[i + (n-1)*meq] = -d[i]; }

        } else  {
            // If not augmented then handle j = n - 1 that is skipped.
            for (CBLAS_INT i = 0; i < meq; i++) { wE[i + (n-1)*meq] = C[i + (n-1)*m]; }

        }
        for (CBLAS_INT i = 0; i < meq; i++) { wf[i] = -d[i]; }
    }
    ...
    // Zero out wG and wh
    for (CBLAS_INT i = 0; i < (mineq + 2*n)*(ld + 1); i++) { wG[i] = 0.0; }
    ...
    if (m > meq)
    {
        for (CBLAS_INT i = 0; i < mineq; i++) { wh[i] = -d[meq + i]; }
    }
    for (CBLAS_INT i = 0; i < n; i++)
    {
        if (isnan(xl[i]))
    ...
    if (m > meq)
    {
        for (CBLAS_INT j = 0; j < orign; j++)
        {
            for (CBLAS_INT i = 0; i < mineq; i++)
            {
    ...
    if (*mode == 1)
    {
        ...
        for (CBLAS_INT i = 0; i < meq; i++) { y[i] = lsei_scratch[i+n_wG_rows]; }
        for (CBLAS_INT i = 0; i < mineq; i++) { y[meq + i] = lsei_scratch[i]; }

        // Set the user-defined bounds on x to NaN
        for (CBLAS_INT i = 0; i < 2*n; i++) { y[m + i] = NAN; }
    }
     ...
     double* xnorm, CBLAS_INT* mode)
{
    ...
    for (CBLAS_INT i = 0; i < n; i++) { x[i] = 0.0; }
    ...
    *mode = 1;
    ...
    if (mg == 0)
    {
        ...
        for (CBLAS_INT i = 0; i < ma; i++) { wb_orig[i] = wb[i]; }
        ...
        for (CBLAS_INT i = 0; i < nvars; i++) { x[i] = wb[i]; }
    ...
    for (CBLAS_INT i = 0; i < mg; i++) { gmults[i] = lsi_scratch[i]; }
...
ORIGINAL_BASIS:
    ...
    for (CBLAS_INT i = 0; i < me; i++) { emults[i] = f[i]; }
...
ldp(CBLAS_INT m, CBLAS_INT n, double* restrict g, double* restrict h, double* restrict x,
    double* restrict buffer, CBLAS_INT* indices, double* xnorm, CBLAS_INT* mode)
{
    CBLAS_INT one = 1;
    ...
    // RHS is (n+1)
    for (CBLAS_INT i = 0; i < n; i++) { b[i] = 0.0; }
    b[n] = 1.0;
    ...
    *xnorm = BLAS_FUNC(dnrm2)(&n, x, &one);
    ...
    for (CBLAS_INT i = 0; i < m; i++) { buffer[i] = fac*y[i]; }
    return;
}
...
ldl_update(CBLAS_INT n, double* restrict a, double* restrict z, double sigma, double* restrict w)
{
    ...
    {
        // Negative update
        for (CBLAS_INT i = 0; i < n; i++) { w[i] = z[i]; }
        for (CBLAS_INT i = 0; i < n; i++)
        {
            v = w[i];
            t = t + v*v/a[ij];
