                ...
                float* restrict work, PROPACK_INT blocksize)
{
    ...
    PROPACK_INT num_full_blocks = n / blocksize;
    PROPACK_INT remainder_cols = n % blocksize;

    // Process full blocks
    for (i = 0; i < num_full_blocks; i++)
    {
        PROPACK_INT block_start = i * blocksize;
    ...
    // Handle remainders
    if (remainder_cols > 0) {
        PROPACK_INT block_start = num_full_blocks * blocksize;
...
void sgemm_ovwr_left(const PROPACK_INT transb, PROPACK_INT m, PROPACK_INT n, PROPACK_INT k, float alpha,
                     float* restrict A, PROPACK_INT lda, float* restrict B, PROPACK_INT ldb,
                     float* restrict work, PROPACK_INT blocksize)
{
    PROPACK_INT i, j, l;
    ...
    // Pre-calculate block structure
    PROPACK_INT num_full_blocks = m / blocksize;
    PROPACK_INT remainder_rows = m % blocksize;
...
void dgemm_ovwr_left(const PROPACK_INT transb, PROPACK_INT m, PROPACK_INT n, PROPACK_INT k, double alpha,
                     double* restrict A, PROPACK_INT lda, double* restrict B, PROPACK_INT ldb,
                     double* restrict work, PROPACK_INT blocksize)
{
    PROPACK_INT i, j, l;
    char transcharb = (transb ? 'T' : 'N');
    double zero = 0.0;
...
static void csgemm_kernel(
    const PROPACK_INT transb, PROPACK_INT m, PROPACK_INT n, PROPACK_INT k, const PROPACK_CPLXF_TYPE* restrict A, PROPACK_INT lda,
    const float* restrict B, PROPACK_INT ldb, PROPACK_CPLXF_TYPE* restrict C, PROPACK_INT ldc)
{
    (void)transb;  // Unused parameter
    // Initialize C to zero
    for (PROPACK_INT j = 0; j < n; j++)
    {
        for (PROPACK_INT i = 0; i < m; i++)
        {
            C[i + j * ldc] = PROPACK_cplxf(0.0f, 0.0f );
        }
    }
    // Always compute C = A * B^T
    for (PROPACK_INT l = 0; l < k; l++)
    {
        for (PROPACK_INT j = 0; j < n; j++)
        {
...
void csgemm_ovwr_left(
    const PROPACK_INT transb, PROPACK_INT m, PROPACK_INT n, PROPACK_INT k, PROPACK_CPLXF_TYPE* restrict A, PROPACK_INT lda,
    const float* restrict B, PROPACK_INT ldb, PROPACK_CPLXF_TYPE* restrict work, PROPACK_INT blocksize)
{
    if ((m <= 0) || (n <= 0) || (k <= 0)) { return; }
    ...
    // Process full blocks
    for (PROPACK_INT i = 0; i < num_full_blocks; i++)
    {
        PROPACK_INT block_start = i * blocksize;
        ...
        for (PROPACK_INT j = 0; j < n; j++)
        {
            for (PROPACK_INT l = 0; l < blocksize; l++)
            {
    ...
    if (remainder_rows > 0)
    {
        ...
        for (PROPACK_INT j = 0; j < n; j++)
        {
            for (PROPACK_INT l = 0; l < remainder_rows; l++)
            {
...
static void zdgemm_kernel(
    const PROPACK_INT transb, PROPACK_INT m, PROPACK_INT n, PROPACK_INT k, const PROPACK_CPLX_TYPE* restrict A, PROPACK_INT lda,
    const double* restrict B, PROPACK_INT ldb, PROPACK_CPLX_TYPE* restrict C, PROPACK_INT ldc)
{
    (void)transb;  // Unused parameter
    // Initialize C to zero
    for (PROPACK_INT j = 0; j < n; j++)
...
void zdgemm_ovwr_left(
    const PROPACK_INT transb, PROPACK_INT m, PROPACK_INT n, PROPACK_INT k, PROPACK_CPLX_TYPE* restrict A, PROPACK_INT lda,
    const double* restrict B, PROPACK_INT ldb, PROPACK_CPLX_TYPE* restrict work, PROPACK_INT blocksize)
{
    if ((m <= 0) || (n <= 0) || (k <= 0)) { return; }
