...
@cython.boundscheck(False)
@cython.cdivision(True)
def evaluate_spline(double[::1] t,     # TODO: binary + on fused types?
             double[:, ::1] c,  #double_or_complex[:,::1] c,
             int k,
             double[::1] xp,
             int dx,
             int extrapolate,
             double[:, ::1] out): #double_or_complex[:,::1] out):
    """
    Evaluate a spline in the B-spline basis.

    Parameters
    ----------
    t : ndarray, shape (n+3k+1)
        knots, augmented
    c : ndarray, shape (n, m)
    ...
    # check derivative order
    if dx < 0:
        raise NotImplementedError("Cannot do derivative order %s." % dx)
    ...
    for ip in range(len(xp)):
        ...
        for jp in range(c.shape[1]):
            ...
            evaluate_bspl(t, k, xval, interval, dx, work)
            out[ip, jp] = 0.
            km = 2*k - interval
            amin = max(0, km)
