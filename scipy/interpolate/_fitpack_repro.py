...
def generate_knots(x, y, *, w=None, xb=None, xe=None, k=3, s=0, nest=None):
    ...
    x, y, w, k, s, xb, xe = _validate_inputs(
        x, y, w, k, s, xb, xe, parametric=y.ndim == 2
    )
...
def root_rati(f, p0, bracket, acc):
    ...
    Note that the latter is for parametric splines and the former is for 1D spline functions.
    The minimization is indentical though [modulo a summation over the dimensions in the
    computation of f(p)], so we reuse the minimizer for both d=1 and d>1.
    """
    # Magic values from
...
def make_splrep(x, y, *, w=None, xb=None, xe=None, k=3, s=0, t=None, nest=None):
    ...
    # postprocess: squeeze the last dimension: was added to simplify _impl internals.
    spl.c = spl.c[:, 0]
    return spl
