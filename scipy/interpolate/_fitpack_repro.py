...
from scipy.interpolate._bsplines import _not_a_knot, make_lsq_spline, fpcheck
...
def generate_knots(x, y, k=3, *, s=0, w=None, nest=None, bbox=(None, None)):
    ...
    if s == 0:
        if nest is not None or w is not None:
            raise ValueError(f"s == 0 is interpolation only")
        t = _not_a_knot(x, k)
        yield t
        return
    ...
    if (x[1:] < x[:-1]).any():
        raise ValueError(f"Expect `x` to be an ordered 1D sequence.")
    k = operator.index(k)

    # XXX: validate bbox (2-tuple of ints or Nones -- what about (None, 42) etc?
