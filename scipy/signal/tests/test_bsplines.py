...
class TestBSplines:
    ...
    def test_spline_filter(self, xp):
        ...
        with pytest.raises(TypeError):
            bsp.spline_filter(xp.asarray([0]), 0)
    ...
    def test_gauss_spline(self, xp):
        ...
        xp_assert_close(bsp.gauss_spline(xp.asarray([1.]), 1),
                        xp.asarray([0.04865217]), atol=1e-8)

    @skip_xp_backends(np_only=True, reasons=["lists inputs are numpy only"])
    def test_gauss_spline_list(self, xp):
        ...
        assert_almost_equal(bsp.gauss_spline(knots, 3),
                            xp.asarray([0.15418033, 0.6909883, 0.15418033]))

    def test_cspline1d(self, xp):
        xp_assert_equal(bsp.cspline1d(xp.asarray([0])), [0.])
    ...
    def test_qspline1d(self, xp):
        ...
        xp_assert_equal(bsp.qspline1d(xp.asarray([0])), [0.])
        # test lamda != 0
        with pytest.raises(ValueError):
            bsp.qspline1d(xp.asarray([1., 2, 3, 4, 5]), 1.)
        ...
        q1d0 = xp.asarray([0.85350007, 2.02441743, 2.99999534, 3.97561055,
                      5.14634135])
        xp_assert_close(bsp.qspline1d(xp.asarray([1., 2, 3, 4, 5])), q1d0)

    def test_cspline1d_eval(self, xp):
        np.random.seed(12464)
        xp_assert_close(bsp.cspline1d_eval(xp.asarray([0., 0]), [0.]), xp.asarray([0.]))
        xp_assert_equal(bsp.cspline1d_eval(xp.asarray([1., 0, 1]), []),
                           xp.asarray([]))
    ...
    def test_qspline1d_eval(self, xp):
        ...
        xp_assert_close(bsp.qspline1d_eval(xp.asarray([0., 0]), [0.]), xp.asarray([0.]))
        xp_assert_equal(bsp.qspline1d_eval(xp.asarray([1., 0, 1]), []),
                           xp.asarray([]))
        x = [-3, -2, -1, 0, 1, 2, 3, 4, 5, 6]
        dx = x[1] - x[0]
        newx = [-6., -5.5, -5., -4.5, -4., -3.5, -3., -2.5, -2., -1.5, -1.,
...
class TestSepfir2d:
    ...
    def test_simple(self, dtyp, xp):
        ...
        expected = xp.asarray([[2.5, 4. , 5.5, 5.5, 4. , 2.5],
                          [2.5, 4. , 5.5, 5.5, 4. , 2.5],
                          [2.5, 4. , 5.5, 5.5, 4. , 2.5],
                          [2.5, 4. , 5.5, 5.5, 4. , 2.5]])
        ...
        expected = xp.asarray([[2., 4., 6., 6., 4., 2.],
                          [2., 4., 6., 6., 4., 2.],
                          [2., 4., 6., 6., 4., 2.],
                          [2., 4., 6., 6., 4., 2.]])
        xp_assert_close(result, expected, atol=1e-16, check_dtype=False)
    ...
    def test_strided(self, dtyp, xp):
        a = xp.asarray([[1, 2, 3, 3, 2, 1, 1, 2, 3],
                     [1, 2, 3, 3, 2, 1, 1, 2, 3],
                     [1, 2, 3, 3, 2, 1, 1, 2, 3],
                     [1, 2, 3, 3, 2, 1, 1, 2, 3]])
        h1, h2 = [0.5, 1, 0.5], [1]
        result_strided = signal.sepfir2d(a[:, ::2], h1, h2)
        result_contig = signal.sepfir2d(xp_copy(a[:, ::2]), h1, h2)
    ...
    def test_sepfir2d_strided_2(self, xp):
        ...
        np.random.seed(1234)
        filt = xp.asarray([1.0, 2.0, 4.0, 2.0, 1.0, 3.0, 2.0])
        image = np.random.rand(4, 4)

        expected = xp.asarray([[36.018162, 30.239061, 38.71187 , 43.878183],
    ...
    def test_sepfir2d_strided_3(self, dtyp, xp):
        ...
        filt = xp.asarray([1, 2, 4, 2, 1, 3, 2], dtype=dtyp)
        image = xp.asarray([[0, 3, 0, 1, 2],
                          [2, 2, 3, 3, 3],
                          [0, 1, 3, 0, 3],
                          [2, 3, 0, 1, 3],
                          [3, 3, 2, 1, 2]], dtype=dtyp)

        expected = xp.asarray([[123., 101.,  91., 136., 127.],
             [133., 125., 126., 152., 160.],
             [136., 137., 150., 162., 177.],
             [133., 124., 132., 148., 147.],
             [173., 158., 152., 164., 141.]])
        ...
        expected = xp.asarray([[22., 35., 41., 31., 47.],
             [27., 39., 48., 47., 55.],
             [33., 42., 49., 53., 59.],
             [39., 44., 41., 36., 48.],
             [67., 62., 47., 34., 46.]])
