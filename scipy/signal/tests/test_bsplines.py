...
from scipy._lib._array_api import (
    xp_assert_close, xp_assert_equal, assert_almost_equal
)
...
class TestBSplines:
    ...
    def test_spline_filter(self):
        ...
        raises(TypeError, bsp.spline_filter, np.array([0]), 0)
    ...
    def test_gauss_spline(self):
        ...
        xp_assert_close(bsp.gauss_spline(np.array([1.]), 1),
                        np.array([0.04865217]), atol=1e-8)
    ...
    def test_gauss_spline_list(self):
        ...
        assert_almost_equal(bsp.gauss_spline(knots, 3),
                            np.array([0.15418033, 0.6909883, 0.15418033]))

    def test_cspline1d(self):
        np.random.seed(12462)
        xp_assert_equal(bsp.cspline1d(np.array([0])), [0.])
        c1d = np.array([1.21037185, 1.86293902, 2.98834059, 4.11660378,
                     4.78893826])
        # test lamda != 0
        xp_assert_close(bsp.cspline1d(np.array([1., 2, 3, 4, 5]), 1), c1d)
        c1d0 = np.array([0.78683946, 2.05333735, 2.99981113, 3.94741812,
                      5.21051638])
        xp_assert_close(bsp.cspline1d(np.array([1., 2, 3, 4, 5])), c1d0)

    def test_qspline1d(self):
        np.random.seed(12463)
        xp_assert_equal(bsp.qspline1d(np.array([0])), [0.])
        # test lamda != 0
        raises(ValueError, bsp.qspline1d, np.array([1., 2, 3, 4, 5]), 1.)
        raises(ValueError, bsp.qspline1d, np.array([1., 2, 3, 4, 5]), -1.)
        q1d0 = np.array([0.85350007, 2.02441743, 2.99999534, 3.97561055,
                      5.14634135])
        xp_assert_close(bsp.qspline1d(np.array([1., 2, 3, 4, 5])), q1d0)

    def test_cspline1d_eval(self):
        np.random.seed(12464)
        xp_assert_close(bsp.cspline1d_eval(np.array([0., 0]), [0.]), np.array([0.]))
        xp_assert_equal(bsp.cspline1d_eval(np.array([1., 0, 1]), []),
                           np.array([]))
        ...
        newx = [-6., -5.5, -5., -4.5, -4., -3.5, -3., -2.5, -2., -1.5, -1.,
                ...
                12.5]
        y = np.array([4.216, 6.864, 3.514, 6.203, 6.759, 7.433, 7.874, 5.879,
                   1.396, 4.094])
        cj = bsp.cspline1d(y)
        newy = np.array([6.203, 4.41570658, 3.514, 5.16924703, 6.864, 6.04643068,
    ...
    def test_qspline1d_eval(self):
        ...
        xp_assert_close(bsp.qspline1d_eval(np.array([0., 0]), [0.]), np.array([0.]))
        xp_assert_equal(bsp.qspline1d_eval(np.array([1., 0, 1]), []),
                           np.array([]))
        x = [-3, -2, -1, 0, 1, 2, 3, 4, 5, 6]
        dx = x[1]-x[0]
        newx = [-6., -5.5, -5., -4.5, -4., -3.5, -3., -2.5, -2., -1.5, -1.,
        ...
        y = np.array([4.216, 6.864, 3.514, 6.203, 6.759, 7.433, 7.874, 5.879,
                   1.396, 4.094])
        cj = bsp.qspline1d(y)
        newy = np.array([6.203, 4.49418159, 3.514, 5.18390821, 6.864, 5.91436915,
...
class TestSepfir2d:
    ...
    def test_simple(self, dtyp):
        ...
        expected = np.array([[2.5, 4. , 5.5, 5.5, 4. , 2.5],
                          [2.5, 4. , 5.5, 5.5, 4. , 2.5],
                          [2.5, 4. , 5.5, 5.5, 4. , 2.5],
                          [2.5, 4. , 5.5, 5.5, 4. , 2.5]])

        xp_assert_close(result, expected, atol=1e-16, check_dtype=False)
        assert result.dtype == sepfir_dtype_map[dtyp]

        result = signal.sepfir2d(a, h2, h1)
        expected = np.array([[2., 4., 6., 6., 4., 2.],
                          [2., 4., 6., 6., 4., 2.],
                          [2., 4., 6., 6., 4., 2.],
                          [2., 4., 6., 6., 4., 2.]])
        xp_assert_close(result, expected, atol=1e-16, check_dtype=False)
    ...
    def test_sepfir2d_strided_3(self, dtyp):
        ...
        expected = np.array([[123., 101.,  91., 136., 127.],
             [133., 125., 126., 152., 160.],
             [136., 137., 150., 162., 177.],
             [133., 124., 132., 148., 147.],
             [173., 158., 152., 164., 141.]])
        ...
        expected = np.array([[22., 35., 41., 31., 47.],
             [27., 39., 48., 47., 55.],
             [33., 42., 49., 53., 59.],
             [39., 44., 41., 36., 48.],
             [67., 62., 47., 34., 46.]])
