...
def get_blas_funcs(names, arrays=(), dtype=None, ilp64="preferred"):
    ...
    See Also
    --------
    get_lapack_funcs : a similar routine for selecting LAPACK functions.
...
for _name, _obj in list(globals().items()):
    if _name[0] in 'sdcz' and not _name.startswith('_'):

        @_functools.wraps(_obj)
        def _wrapper(*args, __n=_name, **kwds):
